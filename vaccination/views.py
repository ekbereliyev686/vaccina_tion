...
class ChooseCampaign(LoginRequiredMixin,generic.ListView):
    ...
    def get_queryset(self):
        return super().get_queryset().filter(vaccine=self.kwargs['vaccine_id']).order_by('start_date')
...
class ConfirmVaccination(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            is_reserved = Slot.reverse_vaccine(self.kwargs['campaign_id'],self.kwargs['slot_id'])
            if is_reserved:
                form.save()
                return HttpResponse("Your vaccine has reserved.")
...
def appointment_letter(request,vaccination_id):
    ...
    context = {'pdf_title':f'{vaccination.patient.get_full_name()}| Appointment Letter',
               'date':str(timezone.now()),
               'title': 'Appointment Letter',
               'subtitle':'Appointment Letter',
               'content':f' This is to in form that the {vaccination.campaign.vaccine.name} vaccintaion of {vaccination.patient.get_full_name()} is scheduled {vaccination.slot.date}',
                         # f'vaccintaion of {vaccination.patient.get_full_name()} is scheduled {vaccination.slot.date}',
                           }
...
def vaccination_certificate(request,vaccination_id):
    ...
    if vaccination.is_vaccinated:
        context ={
            ...
            'subtitle':'To whom may cocern',
            'content': f'This is to certify that the {vaccination.campaign.get_full_name()} has succesfully {vaccination.campaign.vaccine.name} on {vaccination.date} .The vaccination scheduled on {vaccination.campaign.center.name} '
        }
        return generate_pdf(context)
...
def approve_vaccination(request,vaccination_id):
    if request.user.has_perm('vaccination.change_vaccination'):
        ...
        if request.user in vaccination.campaign.agents.all():
            if vaccination.is_vaccinated:
                return HttpResponse('User has already vaccinated')
            vaccination.is_vaccinated = True
            vaccination.date = timezone.now()
            vaccination.updated_date = request.user
            vaccination.save()
            return HttpResponseRedirect(reverse('vaccination:vaccination_detail', kwargs={'pk':vaccination_id}))
